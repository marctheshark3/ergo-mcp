...
async def analyze_transaction(tx_id: str) -> str:
    ...
    try:
        ...
        # Wrap all remaining code in a try-except to catch None comparison errors
        try:
            # Process inputs
            ...
            for i, inp in enumerate(inputs, 1):
                ...
                if assets:
                    ...
                    for asset in assets:
                        ...
                        amount = asset.get("amount", 0) or 0  # Handle None value
            ...
            if all_input_tokens or all_output_tokens:
                ...
                for token_id in all_token_ids:
                    ...
                    # Ensure amounts are never None
                    in_amount = input_amount.get("amount", 0) or 0
                    out_amount = output_amount.get("amount", 0) or 0
                    
                    token_name = input_amount.get("name", "Unknown") if token_id in all_input_tokens else output_amount.get("name", "Unknown")
                    decimals = input_amount.get("decimals", 0) or 0 if token_id in all_input_tokens else output_amount.get("decimals", 0) or 0
            ...
            return f"Error in transaction analysis: {str(inner_e)} (partial result so far: {result})"
